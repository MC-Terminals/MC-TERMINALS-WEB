...
const fecha = new Date(o.fecha_generada).toLocaleDateString('fr-CA');
  ...
  return (
    ...
    (o.piloto || "").toLowerCase().includes(piloto) && // ✅ Previene error
    (filtroBodegaExterna.value === "" || String(o.bodega_externa) === filtroBodegaExterna.value)
  );
});


  // Mostrar total de órdenes encontradas
  document.getElementById("contadorOrdenes").textContent = `Mostrando ${ordenesFiltradas.length} órdenes`;

  // Calcular inicio y fin de la página
  const inicioPagina = (paginaActual - 1) * ordenesPorPagina;
  const finPagina = inicioPagina + ordenesPorPagina;
